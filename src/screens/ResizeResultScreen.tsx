...
type Props = NativeStackScreenProps<RootStackParamList, 'ResizeResult'>;

export function ResizeResultScreen({ navigation, route }: Props) {
  const {
    originalUri,
    originalBytes,
    originalWidth,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.status === 'saved') {
        ...
        Alert.alert('Saved', 'Your resized photo was saved to your gallery.');
        return;
      }
      ...
      if (result.status === 'permission_denied') {
        Alert.alert(
          'Permission needed',
          'Allow SizeKit to save the resized photo to your gallery. The file stays on your device.',
        );
        return;
      }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(resultUri, {
        mimeType: 'image/jpeg',
        dialogTitle: 'Share resized photo',
      });
  ...
  return (
    <Screen scrollable>
      ...
      <View style={styles.stats}>
        ...
        />
        <Stat label="Result" value={`${width}×${height}`} emphasize />
      </View>

      <Text style={styles.meta}>
        {formatFileSize(resultBytes)} ·{' '}
        {mode === 'fit' ? 'Kept original shape' : 'Exact size'}
      </Text>
      ...
      {note ? (
        ...
        <Text style={styles.success}>Ready to use — processed on device.</Text>
      )}
      ...
      />
      <PrimaryButton
        label="Resize another"
        variant="ghost"
        onPress={() => navigation.navigate('ResizePhoto')}
      />
