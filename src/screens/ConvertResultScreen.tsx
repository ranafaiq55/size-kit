...
import { labelForFormat } from '../services/convertFormat';
...
type Props = NativeStackScreenProps<RootStackParamList, 'ConvertResult'>;

export function ConvertResultScreen({ navigation, route }: Props) {
  const {
    originalUri,
    originalBytes,
    resultUri,
  ...
  const formatLabel = labelForFormat(format);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.status === 'saved') {
        ...
        Alert.alert('Saved', `Your ${formatLabel} photo was saved to your gallery.`);
        return;
      }
      ...
      if (result.status === 'permission_denied') {
        Alert.alert(
          'Permission needed',
          'Allow SizeKit to save the converted photo to your gallery. The file stays on your device.',
        );
        return;
      }
  ...
  return (
    <Screen scrollable>
      ...
      <View style={styles.stats}>
        <Stat label="Format" value={formatLabel} emphasize />
        <Stat label="Size" value={formatFileSize(resultBytes)} />
        <Stat label="Pixels" value={`${width}×${height}`} />
      </View>
      ...
      {note ? (
        ...
        <Text style={styles.success}>
          Converted on your device to {formatLabel}.
        </Text>
      )}
      ...
      <PrimaryButton
        ...
        variant="ghost"
        onPress={() => navigation.navigate('ConvertFormat')}
      />
